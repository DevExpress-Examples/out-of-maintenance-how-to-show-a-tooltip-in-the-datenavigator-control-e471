...
Namespace SchedulerTooltip
    Partial Public Class Form1
        ...
        Private selectedDate As Date = Date.MinValue
        Private Sub dateNavigator1_MouseMove(ByVal sender As Object, ByVal e As MouseEventArgs) Handles dateNavigator1.MouseMove
            Dim cp As Point = dateNavigator1.PointToClient(Form1.MousePosition)
            Dim mea As New MouseEventArgs(e.Button, e.Clicks, cp.X, cp.Y, e.Delta)
            Dim chi As CalendarHitInfo = dateNavigator1.GetHitInfo(mea)
        ...
        Private Function GetToolTipInfo(ByVal chi As CalendarHitInfo) As SuperToolTip
            ...
            For Each apt As Appointment In abc
                ...
                item1.Text += String.Format("({0}-{1})" & ControlChars.Lf, apt.Start, apt.End)
            Next apt
